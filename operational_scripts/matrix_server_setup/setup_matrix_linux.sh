...
echo "Copying Synapse docker-compose and synapse_server directory..."
cp -r "$REPO_DIR/synapse_server" "$INSTALL_DIR/synapse_server"
cp "$REPO_DIR/docker-compose.yml" "$INSTALL_DIR/docker-compose.yml"

# Update homeserver.yaml for the new domain
HOMESERVER_YAML="$INSTALL_DIR/synapse_server/data/homeserver.yaml"
...
mv "$INSTALL_DIR/synapse_server/data/${OLD_DOMAIN}.log.config" "$INSTALL_DIR/synapse_server/data/${DOMAIN}.log.config" 2>/dev/null || true
mv "$INSTALL_DIR/synapse_server/data/${OLD_DOMAIN}.signing.key" "$INSTALL_DIR/synapse_server/data/${DOMAIN}.signing.key" 2>/dev/null || true
